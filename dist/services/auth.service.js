...
class AuthService {
    async login(email, password) {
        const user = await store_1.dubikenStore.findUserByEmail(email);
    ...
    async register(data) {
        if (await store_1.dubikenStore.findUserByEmail(data.email)) {
    ...
    me(user) {
        return store_1.dubikenStore.toPublicUser(user);
    }
}
